...
import os from "../language/builtins/index.js";
...
export async function extractDocument(fileName: string, services: LangiumCoreServices): Promise<LangiumDocument> {
  ...
  const builtinDocs = Object.entries(os).map(([uri, text]) => services.shared.workspace.LangiumDocuments.createDocument(URI.parse(uri), text));

  const document = await services.shared.workspace.LangiumDocuments.getOrCreateDocument(URI.file(path.resolve(fileName)));
  await services.shared.workspace.DocumentBuilder.build([...builtinDocs, document], { validation: true });
